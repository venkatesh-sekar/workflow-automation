...
export const jobQueue = (log: FastifyBaseLogger) => ({
    async init(): Promise<void> {
        // Community edition: no dedicated workers, only shared queue
        ...
        switch (type) {
            ...
            case JobType.ONE_TIME: {
                ...
                await queue.add(params.id, data, {
                    priority: JOB_PRIORITY[getDefaultJobPriority(data)],
                    delay: !isNil(dependOnJobId) ? flowDayjsDuration(1, 'year').asMilliseconds() : params.delay,
                    jobId: params.id,
...
async function getQueueName(_platformId: string | null, _log: FastifyBaseLogger): Promise<string> {
    // Community edition: no dedicated workers, all jobs go to shared queue
