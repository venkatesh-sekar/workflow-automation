import { assertNotNullOrUndefined, ConsumeJobResponse, ConsumeJobResponseStatus, JobData, WorkerJobType } from '@flow/shared'
            ...
            return tracer.startActiveSpan('worker.consumeJob', {
                ...
                try {
                    switch (jobData.jobType) {
                        ...
                        default:
                            throw new Error(`Unknown job type: ${(jobData as { jobType: string }).jobType}`)
                    }
                }
                finally {
                    span.end()
