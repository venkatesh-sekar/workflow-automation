import { AppConnectionValue, assertNotNullOrUndefined, UserInteractionJobData, WorkerJobType } from '@flow/shared'
    ...
    async execute(jobData: UserInteractionJobData, engineToken: string, timeoutInSeconds: number): Promise<void> {
        ...
        switch (jobData.jobType) {
            ...
            case WorkerJobType.EXECUTE_PROPERTY:
                log.info({ propertyName: jobData.propertyName, actionOrTriggerName: jobData.actionOrTriggerName }, '[DIAG] executing EXECUTE_PROPERTY')
        ...
        log.info({ requestId: jobData.requestId, workerServerId: jobData.webserverId }, '[DIAG] sending user interaction response')
